...
pub(super) fn derive_decode(item: TokenStream) -> Result<TokenStream> {
    ...
    match input.data {
        Data::Struct(struct_) => {
            let decode_fields = match struct_.fields {
                Fields::Named(fields) => {
                    let init = fields.named.iter().map(|f| {
                        ...
                        quote! {
                            #name: Decode::decode(_rdr).context(#ctx)?,
                        }
                    });
                ...
                Fields::Unnamed(fields) => {
                    let init = (0..fields.unnamed.len())
                        .map(|i| {
                            ...
                            quote! {
                                Decode::decode(_rdr).context(#ctx)?,
                            }
                        })
                        .collect::<TokenStream>();
            ...
            add_trait_bounds(
                &mut input.generics,
                quote!(crate::__private::Decode<#lifetime>),
            );
            ...
            Ok(quote! {
                impl #impl_generics crate::__private::Decode<#lifetime> for #input_name #ty_generics
                #where_clause
                {
                    fn decode(_rdr: &mut &#lifetime [u8]) -> crate::__private::Result<Self> {
        ...
        Data::Enum(enum_) => {
                        ...
                        Fields::Named(fields) => {
                            let fields = fields
                                ...
                                .map(|f| {
                                    ...
                                    quote! {
                                        #field: Decode::decode(_rdr).context(#ctx)?,
                                    }
                                })
                                .collect::<TokenStream>();
            ...
            Ok(quote! {
                ...
                {
                    fn decode(_rdr: &mut &#lifetime [u8]) -> crate::__private::Result<Self> {
                        use crate::__private::{Decode, Context, VarInt, bail};

                        let ctx = concat!("failed to decode enum discriminant in `", stringify!(#input_name), "`");
                        let disc = VarInt::decode(_rdr).context(ctx)?.0;
                        match disc {
                            #decode_arms
